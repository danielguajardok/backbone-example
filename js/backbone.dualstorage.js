...
(function() {
  ...
  window.Store = (function() {
    ...
    Store.prototype.setTime = function() {
      return localStorage.setItem(this.name + "_time", new Date().getTime());
    };

    Store.prototype.getTime = function() {
      return localStorage.getItem(this.name + "_time");
    };
  ...
  dualsync = function(method, model, options) {
    var comparator, error, last_sync, local, now, originalModel, store, success;
    console.log('dualsync', method, model, options);
    options.storeName = (result(model.collection, 'url') || result(model, 'url')) + (options.data ? "?"+options.data : "")
    ...
    switch (method) {
      case 'read':
        if (localsync('hasDirtyOrDestroyed', model, options)) {
          ...
          comparator = (model.collection ? (model.collection.cache ? model.collection.cache.ttl || 0 : 0) : (model.cache ? model.cache.ttl || 0 : 0));
          console.log(comparator);
          last_sync = store.getTime() || 0;
